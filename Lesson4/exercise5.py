...
ipv6_addr = re.search(r"IPv6 address:.*?(\S+/\d+).*?(\S+/\d+).*IPv6 subnet", output, flags=re.DOTALL).groups()
ipv6_list = list(ipv6_addr)

print()
print("IPv6 addresses list: {}".format(ipv6_list))
print()
